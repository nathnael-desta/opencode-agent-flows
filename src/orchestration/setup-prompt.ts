...
export const SETUP_COMMAND_TEMPLATE = [
  ...
  "",
  "7. Call flow_antigravity, then lay out the optional add-ons as a short menu so the user can see the whole ecosystem in one place and pick what they want. For each, give the one-line reason and the exact command:\n   - Operating-policy skills (recommended, free, loads on demand):\n     npx skills@latest add nathnael-desta/skills --skill browser-control-operations --skill antigravity-delegation --agent opencode --global -y\n   - Gemini via Antigravity, for vision and large-context reads on a Google AI Pro subscription: install the agy CLI from https://antigravity.google/cli/install.sh, run agy once to log in, then add the opencode-antigravity-delegate plugin. Its tools then appear automatically and this orchestrator will route perception and bulk work to them.\n   - Command Code, for low-cost open-source worker models: add the opencode-commandcode-provider plugin and set COMMANDCODE_API_KEY.\n   - Repository workflow setup (GitHub Issues plus Beads):\n     npx skills@latest add nathnael-desta/skills --skill setup-github-beads --agent opencode --global -y\n     then run the setup-github-beads skill inside the repository you want configured.\n   State plainly which of these the user already has, based on flow_antigravity and the providers you discovered, so they only act on what is missing. Do not change their role bindings for any of this; these are complementary helpers, not role models.",

  ...
  agents: {
    orchestrator: {
      ...
      prompt: [
        ...
        "If antigravity_delegate, antigravity_vision, or antigravity_background_start tools are available, load the antigravity-delegation skill as operating policy before delegating: Gemini substitutes for suitable read-only work such as vision, large-context reads, and cheap bulk triage, and never becomes a mandatory stage, a duplicate of a routine worker, or a step on the agentic loop, escalation, or milestone review on your primary models, because Gemini Flash is weak at long-horizon autonomy. Route only Google Gemini models through Antigravity; non-Google models there add that vendor terms layer.",
        "When Browser Control MCP tools are available, load the browser-control-operations skill as operating policy and use MCP as the execution transport. Browser Control is the default browser transport; keep inspect, act, and verify in the root orchestration loop, prefer the user's attached authenticated session, and require explicit confirmation before destructive or account-changing actions.",

...
- [ ] Use a task with no automated verification and confirm reviewer behavior.
- [ ] Use a failed-then-fixed task and confirm the reviewer receives final state.
...
- [ ] Confirm a reviewer failure does not fail the production run.
...
- [ ] Confirm ordinary `deep` delegation does not require this approval.
